 ...
 * Serve Trellis Chart em /trellis e Boxplot Chart em /boxplot (paths fixos).
 * Assets servidos apenas por express.static em /trellis e /boxplot; não existe rota dinâmica em /assets.
 * rejectPathTraversal protege os mounts estáticos contra path traversal.
 */
...
// Middleware para bloquear Path Traversal antes do express.static (ex: /trellis/assets/..%2f..%2f)
function rejectPathTraversal() {
    return (req: express.Request, res: express.Response, next: express.NextFunction) => {
        const decoded = decodeURIComponent(req.path);
        if (decoded.includes('..') || decoded.includes('%2e%2e') || /%2e/i.test(decoded)) {
            return res.status(400).setHeader('Content-Type', 'text/plain').send('Bad request');
        }
        next();
    };
}
app.use('/trellis', rejectPathTraversal());
app.use('/trellis', express.static(trellisDistPath, { index: false }));

// Resposta explícita para /assets: não existe rota dinâmica; uso correto é /trellis ou /boxplot.
app.get('/assets', (_req, res) => {
    res.status(404).setHeader('Content-Type', 'text/plain').send('Use /trellis or /boxplot');
});
...
app.use('/boxplot', rejectPathTraversal());
